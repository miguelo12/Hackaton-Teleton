...
class AlumnoSeccion {
    ...
    public function EntregarSeccionAlumno()
    {
      ...
      $sentencia=$c->prepare("select asi.Nombre as asignatura, unidad.Titulo as unidadNombre , aluse.Seccion_idSeccion as idseccion, act.idActividad as idactividad from alumnoseccion as aluse inner JOIN seccion as se on se.idSeccion=aluse.Seccion_idSeccion inner join asignatura as asi on se.Asignatura_idAsignatura=asi.idAsignatura inner join actividad as act on act.Seccion_idSeccion=se.idSeccion inner join unidadaprendizaje as unidad on act.UnidadAprendizaje_idUnidadAprendizaje=unidad.idUnidadAprendizaje where aluse.Alumno_idAlumno=? and act.finalizada=0 and act.fecha_termino>=CURDATE()");
